 ...
 */
const formatName = (firstName?: string, lastName?: string): string | undefined => {
 ...
 */
const formatProbableEmail = (name: string, company: string): string => {
 ...
 */
const calculateLeadScore = (params: {
  industry: string;
  location: string;
  hasEmail: boolean;
  hasPhone: boolean;
 ...
 */
const calculateStoreScore = (params: {
  storeType: string;
  hasEmail: boolean;
  hasPhone: boolean;
  location: string;
 ...
 */
const removeDuplicates = (leads: Partial<B2BLead>[]): Partial<B2BLead>[] => {
 ...
 */
const generateEmailTemplate = (industry: string): string => {
...
return [{
  ...
  Object.keys(leadsByIndustry).forEach((industry, index) => {
    ...
    if (!workflow.connections[`${industry} Segment`]) {
      workflow.connections[`${industry} Segment`] = { main: [[]] };
    }
    workflow.connections[`${industry} Segment`].main[0].push({
  ...
  Object.keys(leadsByIndustry).forEach((industry, index) => {
    ...
    if (!workflow.connections[`${industry} Email Template`]) {
      workflow.connections[`${industry} Email Template`] = { main: [[]] };
    }
    workflow.connections[`${industry} Email Template`].main[0].push({
...
return [{
  ...
  Object.keys(leadsByIndustry).forEach((industry) => {
    if (!workflow.connections[`Send to ${industry}`]) {
      workflow.connections[`Send to ${industry}`] = { main: [[]] };
    }
    workflow.connections[`Send to ${industry}`].main[0].push({
